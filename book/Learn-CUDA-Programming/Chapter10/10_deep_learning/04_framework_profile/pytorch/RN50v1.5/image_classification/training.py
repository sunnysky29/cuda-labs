...
def get_train_step(model_and_loss, optimizer, fp16, use_amp = False, batch_size_multiplier = 1):
    def _step(input, target, optimizer_step = True):
        ...
        torch.cuda.nvtx.range_pop() # NVTX: forward
        ...
        torch.cuda.nvtx.range_pop() # NVTX: reduce
        ...
        if fp16:
            ...
            loss.backward()
        torch.cuda.nvtx.range_pop() # NVTX: backward
        ...
        if optimizer_step:
            ...
            optimizer.step()
            optimizer.zero_grad()
        torch.cuda.nvtx.range_pop() # NVTX: optimize
...
def train(train_loader, model_and_loss, optimizer, lr_scheduler, fp16, logger, epoch, use_amp=False, prof=-1, batch_size_multiplier=1, register_metrics=True):
    ...
    for i, (input, target) in data_iter:
        # NVTX: displaying step index
        torch.cuda.nvtx.range_push("step:" + str(i))
        ...
        torch.cuda.nvtx.range_pop() # NVTX:step index
