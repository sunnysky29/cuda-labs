...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bn1 is not None:
            x = self.bn1(x)
        x = self.relu(x)
        torch.cuda.nvtx.range_pop() # NVTX: conv1

        torch.cuda.nvtx.range_push("conv2")
        x = self.maxpool(x)
        x = self.layer1(x)
        torch.cuda.nvtx.range_pop() # NVTX: conv2

        torch.cuda.nvtx.range_push("conv3")
        x = self.layer2(x)
        torch.cuda.nvtx.range_pop() # NVTX: conv3

        torch.cuda.nvtx.range_push("conv4")
        x = self.layer3(x)
        torch.cuda.nvtx.range_pop() # NVTX: conv4

        torch.cuda.nvtx.range_push("conv5")
        x = self.layer4(x)
        torch.cuda.nvtx.range_pop() # NVTX: conv5

        torch.cuda.nvtx.range_push("fc")
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        torch.cuda.nvtx.range_pop() # NVTX: fc
