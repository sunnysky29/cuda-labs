...
nvcc  test_bxtx.cu -o test_bxtx  &&  ./test_bxtx
...
/*------------------核函数启动------------------------*/
    ...
    MatrixMulKernel_multiblock<<<grid, block>>>(Ad,Bd,Cd,width);

    cudaDeviceSynchronize();
    iElaps = (double)(clock() - iStart) / CLOCKS_PER_SEC;;
    printf("device矩阵乘 on device <<<%d, %d>>>：\t %.2f μs\n", 
        grid.x, block.x, iElaps*1e6);
    printf("矩阵尺寸为：%dx%d\n",width,width);
