...
开启多个block, 2D 形式
nvcc  test_bxtx2D.cu -o test_bxtx2D  &&  ./test_bxtx2D
...
__global__ void MatrixMulKernel_2Dmultiblock(float* Ad, float* Bd, float* Cd, int width)
{
    int row = blockIdx.y*blockDim.y+threadIdx.y;
    int col = blockIdx.x*blockDim.x+threadIdx.x;
    
    if (row >= width || col >= width) return;  // 边界检查
...
/*------------------设置线程布局------------------------*/
dim3 block(16, 64);  // 256 threads per block
dim3 grid((width + block.x - 1) / block.x, 
          (width + block.y - 1) / block.y);

printf("Launching <<<(%d,%d), (%d,%d)>>>\n", 
       grid.x, grid.y, block.x, block.y);
/*------------------核函数启动------------------------*/

    iStart = clock();
    // MatrixMulKernel<<<1, 1024>>>(Ad,Bd,Cd,width);
    MatrixMulKernel_2Dmultiblock<<<grid, block>>>(Ad,Bd,Cd,width);

    cudaDeviceSynchronize();
    iElaps = (double)(clock() - iStart) / CLOCKS_PER_SEC;;
    printf("device矩阵乘 <<<(%d,%d), (%d,%d)>>> \t %.2f ms\n", 
        grid.x, grid.y, block.x, block.y, iElaps*1e3);
    printf("矩阵尺寸为：%dx%d\n",width,width);
